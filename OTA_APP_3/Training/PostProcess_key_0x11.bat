...
copy /b %FILE_NAME_APP%_key_0x11.bin + %FILE_NAME_APP%.bin %FILE_NAME_APP%_key_temp.bin

:: 명령어 실행 및 결과 가져오기
for /f "tokens=*" %%i in ('CertUtil -hashfile %FILE_NAME_APP%_key_temp.bin SHA256') do (
    set /a count+=1
    set "line[!count!]=%%i"
)
:: 3번째 행 출력
...
!CMD_APP_SX_2!
set CMD_SIGNATURE=%HEXVIEW_PATH% %FILE_NAME_APP%_temp.sx /s /FR:!APP_ADDR_SIGN!,32 /FP:%signature% /XS:32 -o %FILE_NAME_APP%_key_sign.sx
!CMD_SIGNATURE!
set CMD_APP_BIN_SIGN=%HEXVIEW_PATH% %FILE_NAME_APP%_key_sign.sx /s /XN -o %FILE_NAME_APP%_key_sign.bin
!CMD_APP_BIN_SIGN!

del %FILE_NAME_APP%_temp.sx
del %FILE_NAME_APP%_key_temp.bin
